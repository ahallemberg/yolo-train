...
class Config():
    ...
    def __init__(self, u: str):
        if not self._initialized:
            ...
            if u == "tune":
                self.load_params("tune.params.yaml", ["name", "data", "project", "epochs", "iterations"])
            elif u == "train":
                self.load_params("train.params.yaml", ["name", "data", "project", "epochs"])
            else: 
    ...
    def load_params(self, base_name: str, required: list[str]):
        target_file = os.path.abspath(os.path.join(os.path.dirname(__file__), base_name))
